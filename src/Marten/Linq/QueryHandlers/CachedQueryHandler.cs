...
namespace Marten.Linq.QueryHandlers
{
    internal class CachedQueryHandler<T>: IQueryHandler<T>
    {
        ...
        private readonly SqlCommand _template;
        private readonly IQueryHandler<T> _handler;
        private readonly IDbParameterSetter[] _setters;
        private readonly ISerializer _serializer;

        public CachedQueryHandler(object model, SqlCommand template, IQueryHandler<T> handler, IDbParameterSetter[] setters, ISerializer serializer)
        {
            _model = model;
            _template = template;
            _handler = handler;
        ...
        public void ConfigureCommand(CommandBuilder builder)
        {
            ...
            for (var i = 0; i < _setters.Length && i < _template.Parameters.Count; i++)
            {
                ...
                {
                    param.Value = _serializer.EnumStorage == EnumStorage.AsInteger ? (int)param.Value : param.Value;
                }

                param.SqlDbType = _template.Parameters[i].SqlDbType;
